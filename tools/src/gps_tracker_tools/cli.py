...
Commands:
    gt fetch       - Apple Find My location query and GPX export
...
from gps_tracker_tools import findmy, casic, gps_format, uf2
from gps_tracker_tools.findmy import DEFAULT_ANISETTE_URL, DEFAULT_AUTH_PATH
from gps_tracker_tools.gpx import dedupe_reports, write_gpx


def cmd_fetch(args):
    """Fetch Apple Find My reports, dedupe, and export."""
    if not args.findmy_keys:
        print("Error: --findmy-keys is required", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        all_results = findmy.cmd_fetch(apple_args) or []
        for r in all_results:
            r["source"] = "apple"
        print(f"Apple: {len(all_results)} locations", file=sys.stderr)
    except Exception as e:
        print(f"Apple fetch failed: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    # Dedupe reports before writing or printing.
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""\
Examples:
  gt fetch --findmy-keys keys.json --gpx track.gpx
...
""",
    ...
    p_fetch = sub.add_parser(
        "fetch",
        help="Apple Find My location fetch",
    )
