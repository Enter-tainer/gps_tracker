...
# Build (default: logging via RTT, requires debug probe)
cargo build --release

# Build with USB serial logging (no debug probe needed)
cargo build --release --features usb-serial-log
...
- Logging via `defmt`: default RTT (debug probe), optional USB serial with `usb-serial-log` feature
